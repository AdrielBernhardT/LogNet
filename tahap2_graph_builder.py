...
print("1. Memuat data Auth yang sudah di-sampling...")
...
df_auth_graph = pd.DataFrame({
    ...
    'edge_type': 0,                   
    'is_success': df_auth['is_success'], # Bawa fitur sakti
    'label': df_auth['label']         
})
del df_auth
gc.collect()

print("2. Memuat sampel data Flows (NetFlow)...")
flow_cols = ['time', 'duration', 'src_comp', 'src_port', 'dst_comp', 'dst_port', 'protocol', 'packet_count', 'byte_count']
# Ambil 500k baris demi menjaga RAM laptopmu (mengingat RAM sempat 85%)
...
df_flow_graph = pd.DataFrame({
    ...
    'edge_type': 1,                   
    'is_success': 1,                  # Flow kita anggap koneksi sukses
    'label': 0                        
})
del df_flow_raw
gc.collect()
...
# Susun ulang kolom
df_graph = df_graph[['time', 'src_id', 'dst_id', 'edge_type', 'is_success', 'label']]

...
# === TAMBAHAN: Fail-fast check — pastikan engine parquet tersedia SEBELUM
# memproses 1 milyar+ baris. Tanpa ini, script bisa crash di baris terakhir
# setelah berjam-jam processing (seperti yang terjadi sebelumnya). ===
try:
    ...
    try:
        ...
        sys.exit(
            "\n[FATAL] Tidak ada engine parquet yang terpasang (pyarrow/fastparquet).\n"
            "Install dulu sebelum menjalankan script ini (proses chunking 1B+ baris "
            "akan sia-sia kalau gagal di langkah save):\n\n"
...
# === TAMBAHAN: fallback ke pickle kalau parquet gagal di menit terakhir
# (mis. versi pyarrow bermasalah) — supaya hasil 1B+ baris tidak hilang lagi. ===
try:
    final_df.to_parquet(OUTPUT_FILE, index=False, engine=PARQUET_ENGINE)
    print(f"File berhasil disimpan di: {OUTPUT_FILE}")
except Exception as e:
    fallback_file = OUTPUT_FILE.rsplit('.', 1)[0] + '.pkl'
    print(f"[WARNING] Gagal menyimpan parquet ({e}). Menyimpan fallback ke pickle...")
    final_df.to_pickle(fallback_file)
    print(f"File fallback berhasil disimpan di: {fallback_file}")
    print("(Baca dengan pd.read_pickle() di tahap berikutnya, atau convert manual ke parquet nanti.)")

...
# V4.2: train_test_split (random stratified) sudah tidak dipakai -> diganti chronological split (lihat bagian 5)
import numpy as np
import json
import matplotlib
matplotlib.use('Agg')
...
# ==========================================
# 5. PEMISAHAN VALIDASI & TESTING (CHRONOLOGICAL - V4.2)
# ==========================================
# V4.2: sebelumnya val/test di-split RANDOM (stratified) di dalam window "masa
# depan" -> diganti CHRONOLOGICAL. Alasannya: simulasi jadi lebih mirip
# deployment nyata (threshold di-tuning pakai paruh AWAL masa depan, seolah
# "data kemarin", lalu diuji murni di paruh AKHIR, seolah "data hari ini" yang
# belum pernah disentuh sama sekali) -- bukan cuma "belum dipakai training".
...
val_test_boundary = int(0.50 * len(future_preds_prob))

val_preds_prob = future_preds_prob[:val_test_boundary]
val_true_labels = future_true_labels[:val_test_boundary]
val_is_auth = future_is_auth[:val_test_boundary]

test_preds_prob = future_preds_prob[val_test_boundary:]
test_true_labels = future_true_labels[val_test_boundary:]
test_is_auth = future_is_auth[val_test_boundary:]

print(f"[OK] Validasi/Test dibagi CHRONOLOGICAL -> Validasi (paruh awal 'masa depan'): {len(val_preds_prob)} | Ujian Akhir/Test (paruh akhir 'masa depan'): {len(test_preds_prob)}")
...
# ==========================================
# 8. BOOTSTRAP CONFIDENCE INTERVAL UNTUK ESTIMASI DUNIA NYATA (V4.2 - TAMBAHAN)
# ==========================================
# Kenapa perlu: flow_frac cuma ~0.005 (0.5%), jadi tiap FP yang keobservasi di
# flow edge dikali ~200x saat dikoreksi (inverse probability weighting).
# Artinya angka estimasi di atas bisa sangat noisy kalau jumlah flow edge yang
# salah diklasifikasi di test set kecil. Bootstrap resampling (dengan
# replacement) dipakai untuk mengukur seberapa lebar ketidakpastian itu.
print("\n" + "=" * 40)
print("BOOTSTRAP CONFIDENCE INTERVAL (95%) UNTUK ESTIMASI DUNIA NYATA")
print("=" * 40)
...
boot_precision, boot_recall, boot_f1, boot_fp = [], [], [], []

for _ in range(N_BOOTSTRAP):
    boot_idx = rng.integers(0, n_test, size=n_test)  # resample dengan replacement
    bt_true = test_true_labels[boot_idx]
    bt_pred = final_test_preds[boot_idx]
    bt_is_auth = test_is_auth[boot_idx]

    hasil_boot = hitung_confusion_terkoreksi(bt_true, bt_pred, bt_is_auth, auth_frac, flow_frac)
    b_tp, b_fn, b_fp_est = hasil_boot['TP'], hasil_boot['FN'], hasil_boot['FP_estimasi']

    b_prec = b_tp / (b_tp + b_fp_est) if (b_tp + b_fp_est) > 0 else 0.0
    b_rec = b_tp / (b_tp + b_fn) if (b_tp + b_fn) > 0 else 0.0
    b_f1 = (2 * b_prec * b_rec / (b_prec + b_rec)) if (b_prec + b_rec) > 0 else 0.0

    boot_precision.append(b_prec)
    boot_recall.append(b_rec)
    boot_f1.append(b_f1)
    boot_fp.append(b_fp_est)
...
prec_lo, prec_hi = ci95(boot_precision)
rec_lo, rec_hi = ci95(boot_recall)
f1_lo, f1_hi = ci95(boot_f1)
fp_lo, fp_hi = ci95(boot_fp)

print(f"Precision (estimasi) : {precision_est*100:.2f}%  (95% CI: {prec_lo*100:.2f}% - {prec_hi*100:.2f}%)")
print(f"Recall    (estimasi) : {recall_est*100:.2f}%  (95% CI: {rec_lo*100:.2f}% - {rec_hi*100:.2f}%)")
print(f"F1-score  (estimasi) : {f1_est:.4f}  (95% CI: {f1_lo:.4f} - {f1_hi:.4f})")
print(f"FP        (estimasi) : {fp_est:,.0f}  (95% CI: {fp_lo:,.0f} - {fp_hi:,.0f})")
print(
    f"\n[INTERPRETASI] Lebar interval FP ({fp_hi - fp_lo:,.0f}) mencerminkan seberapa sensitif "
    f"angka 'estimasi dunia nyata' terhadap variasi sampling flow edge (flow_frac={flow_frac:.4f}). "
    "Kalau intervalnya lebar, laporkan CI ini juga di paper/laporan, jangan cuma titik estimasi tunggal."
)
...
# ==========================================
# 9. SIMPAN SEMUA HASIL AKHIR KE JSON (V4.2 - TAMBAHAN)
# ==========================================
# Supaya angka-angka ini tidak hilang kalau console log tidak disimpan --
# dan supaya bisa dikutip langsung di laporan/paper tanpa re-run.
hasil_akhir = {
    "threshold": {
        "deterministik": float(threshold_deterministik),
        "pso_cross_check": float(gbest_position),
        "selisih_relatif_persen": float(selisih_pct),
        "dipakai_untuk_evaluasi": float(threshold_final),
        "target_recall_validasi": TARGET_RECALL,
    },
    "split_info": {
        "metode_val_test": "chronological",
        "n_val": int(len(val_preds_prob)),
        "n_test": int(len(test_preds_prob)),
        "n_attack_val": int(total_attack_di_val),
        "n_attack_test": int(total_attack_di_test),
    },
    "test_set_observed": {
    ...
    "estimasi_dunia_nyata": {
        "sampling_fraction_auth": float(auth_frac),
        "sampling_fraction_flow": float(flow_frac),
        "TP": int(tp), "FN": int(fn),
        "TN_estimasi": float(tn_est), "FP_estimasi": float(fp_est),
        "precision": float(precision_est),
        "recall": float(recall_est),
        "f1": float(f1_est),
        "bootstrap_95ci": {
...
with open(RESULT_FILE, 'w') as f:
    json.dump(hasil_akhir, f, indent=2)

print(f"\n[TERSIMPAN] Semua hasil evaluasi akhir (threshold, confusion matrix, estimasi, bootstrap CI) disimpan di: {RESULT_FILE}")